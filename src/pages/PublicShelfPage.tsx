...
export default function PublicShelfPage() {
  ...
  useEffect(() => {
    if (!slug) return;
    apiFetch<PublicShelf>(`/shelves/public/shelf/${slug}`, null)
      .then(setData)
      .catch(() => setError('Shelf not found or is private.'));
  }, [slug]);
  ...
  const ownerName = data.user.name || data.user.email;

  return (
    <div className="min-h-screen bg-gray-950 text-gray-100 px-6 py-12 max-w-5xl mx-auto">
      <p className="text-xs text-gray-500 uppercase tracking-wide">Shared shelf</p>
      <h1 className="mt-2 text-2xl font-bold text-white">{data.name}</h1>
      <p className="mt-1 text-sm text-gray-400">by {ownerName}</p>
      <p className="mt-1 text-xs text-gray-600">{data.books.length} book{data.books.length !== 1 ? 's' : ''}</p>
      ...
      <div className="mt-10 relative">
        <div className="flex items-end gap-4 px-5 pb-0 min-h-[200px] flex-wrap">
          ...
          {data.books.length === 0 && (
            <p className="text-sm text-gray-600 py-16">This shelf is empty.</p>
          )}
        </div>
